...
def create_tvshows_from_xml():
    ...
    if filetools.exists(fname):
        platformtools.dialog_ok("Biblioteca: verrà aggiornata al nuovo formato",
                                "Selezionare il nome corretto per ogni serie, e se non siete sicuri 'Annullate'.",
                                "Ci sono nuove opzioni in 'Libreria' e in 'configurazione.")
    ...
    if filetools.exists(fname):
        if not _actualizado:
            platformtools.dialog_ok("Libreria: Aggiornamento formato"
                                     "Per favore attendi che il processo sia completato")
