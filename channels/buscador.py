...
def mainlist(item, preferred_thumbnail="squares"):
    ...
    itemlist = list()
    itemlist.append(Item(channel=item.channel, action="search", title="Ricerca per titolo..."))
...
def opciones(item):
    ...
    itemlist.append(Item(channel=item.channel, action="settings", title="Impostazioni"))
    return itemlist
...
def settingCanal(item):
    ...
    return platformtools.show_channel_settings(list_controls=list_controls,
                                               caption="I canali inclusi nella ricerca globale",
...
def searchbycat(item):
    ...
    categories = ["Film", "Serie", "Anime", "Documentati", "VOS"]
    categories_id = ["movie", "serie", "anime", "documentary", "vos" ]
    list_controls = []
    ...
    control = {'id': "torrent",
               'type': "bool",
               'label': 'Includere nella ricerca i canali Torrent',
               'default': True,
               'enabled': True,
               'visible': True}
    list_controls.append(control)

    return platformtools.show_channel_settings(list_controls=list_controls, caption="scegli la categoria",
...
def do_search(item, categories=[]):
    ...
    time.sleep(0.5)
    progreso = platformtools.dialog_progress("Sto cercando " + tecleado, "")
    ...
    if multithread:
        progreso.update(0, "trovato %s..." % (tecleado))
    ...
    for index, infile in enumerate(channel_files):
        ...
        # Modo single Thread
            ...
            logger.info(
                "pelisalacarta.channels.buscador Intentado busqueda en " + basename_without_extension + " de " + tecleado)
    ...
    if multithread:
        ...
        while pendent:
            ...
            progreso.update(percentage, "Sto cercando %s in %d canali..." % (tecleado, len(searches)))
    ...
    for channel in sorted(search_results.keys()):
        for search in search_results[channel]:
            ...
            if result_mode == 0:
                ...
                itemlist.append(
                    Item(title=title, channel="buscador", action="channel_result", url=search["item"].url, extra=extra,
                         folder=True))
            else:
                itemlist.extend(search["itemlist"])

    title = "Ho trovato: '%s' | trovati: %d titoli | in: %2.f secondi" % (tecleado, total, time.time() - start_time)
